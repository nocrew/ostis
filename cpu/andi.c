...
static void andi_b(struct cpu *cpu, WORD op)
{
  BYTE s,d,r;
...
static void andi_w(struct cpu *cpu, WORD op)
{
  WORD s,d,r;
...
static void andi_l(struct cpu *cpu, WORD op)
{
  LONG s,d,r;
...
static void andi(struct cpu *cpu, WORD op)
{
  ENTER;
...
static struct cprint *andi_print(LONG addr, WORD op)
{
  int s;
  struct cprint *ret;
...
void andi_init(void *instr[], void *print[])
{
  int i;
  for(i=0;i<0xc0;i++) {
    instr[0x0200|i] = andi;
    print[0x0200|i] = andi_print;
  }
}
