...
static void or_b(struct cpu *cpu, WORD op)
{
  BYTE r;
  int reg;
...
static void or_w(struct cpu *cpu, WORD op)
{
  WORD r;
  int reg;
...
static void or_l(struct cpu *cpu, WORD op)
{
  LONG r;
  int reg;
...
static void or(struct cpu *cpu, WORD op)
{
  ENTER;
...
static struct cprint *or_print(LONG addr, WORD op)
{
  ...
  switch(s) {
  case 0:
    strcpy(ret->instr, "OR.B");
    break;
  case 1:
    strcpy(ret->instr, "OR.W");
    break;
  case 2:
    strcpy(ret->instr, "OR.L");
    break;
  }
...
void or_init(void *instr[], void *print[])
{
  int i,r;
