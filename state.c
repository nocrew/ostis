...
struct state *state_collect()
{
  ...
  new_list = xmalloc(sizeof(struct state_list));
...
struct state *state_encode_delta(struct state *state, struct state *ref)
{
  ...
  tmp = xmalloc(state->cpu_state->size);
...
struct state *state_decode_delta(struct state *coded, struct state *ref)
{
  ...
  tmp = xmalloc(ref->cpu_state->size);
  if(tmp == NULL) return NULL;
...
struct state *state_load(char *filename)
{
  ...
  while(state_read_section(f, &sec)) {
	...
	return NULL;
      ...
      new->cpu_state = xmalloc(sizeof(struct cpu_state));
      if(new->cpu_state == NULL) {
	free(new);
	free(sec.data);
	return NULL;
      }
      new->cpu_state->data = xmalloc(sec.size);
	...
	return NULL;
      ...
      t = xmalloc(sizeof(struct mmu_state));
      if(t == NULL) {
	free(new);
	free(sec.data);
	return NULL;
      }
      t->data = xmalloc(sec.size);
      if(t->data == NULL) {
	free(t);
	free(new);
	free(sec.data);
